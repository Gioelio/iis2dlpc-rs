    ...
    I: InterruptPin
{
    ...
    sensor.tap_threshold_x_set(12).await.unwrap();
    sensor.tap_threshold_y_set(12).await.unwrap();
    sensor.tap_threshold_z_set(12).await.unwrap();
    // Configure Double Tap parameter
    sensor.tap_dur_set(7).await.unwrap();
    sensor.tap_quiet_set(3).await.unwrap();
    sensor.tap_shock_set(3).await.unwrap();
    ...
    loop {
        ...
        let tap_src = all_sources.tap_src;

        if tap_src.double_tap() == 1 {
            core::write!(tx, "Double tap Detected: Sign ").unwrap();
        } else if tap_src.single_tap() == 1 {
            core::write!(tx, "Single tap Detected: Sign ").unwrap();
        } else {
            continue;
        }

        if tap_src.tap_sign() == 1 {
